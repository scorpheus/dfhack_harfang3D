...
class building_type():
	...
	WaterWheel, Windmill, TractionBench, Slab, Nest, NestBox, Hive, Rollers, Instrument, Bookcase = range(53)
...
def setup():
	...
	building_geos = {building_type.Chair: None, building_type.Bed: None,
					 ...
					 building_type.Hive: None, building_type.Rollers: None, building_type.Instrument: None, building_type.Bookcase: None}
	...
	for ramp in ramp_to_evaluate:
		if 0 < ramp[0] < 16 and 0 < ramp[1] < 16:
			...
			if id_ramp not in ramp_geos:
				...
				iso = hg.IsoSurface()
				hg.PolygoniseIsoSurfaceBinaryData(w+2, h+2, d+3, field, 1.0, iso, vec3(1/2, 1, 1/2))
